...
1. 节点 `spread` 调度策略，请看[示例](../sources/node-spread.yaml)：
...
1. 卡级别 `binpack` 调度策略，请看示例[](../sources/gpu-binpack.yaml):
...
1. 卡级别 `spread` 调度策略，请看示例[](../sources/gpu-spread.yaml):
...
1. 节点级别和卡级别的 `spread`，请看[示例](../sources/gpu-node-spread.yaml):
...
1. HAMi 节点默认的调度策略是 `binpack`，GPU 的调度策略是 `spread`。可以通过修改启动参数来修改默认的行为：
